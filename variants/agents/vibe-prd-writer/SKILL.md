...
- `~/.Codex/skills/vibe-prd-writer/references/module-specs.md`——定义 10 个模块的完整生成规范。Phase 4 写 PRD 时按这个模板输出。
- `~/.Codex/skills/vibe-prd-writer/references/style-guide.md`——定义前端风格定调的 4 套方案生成流程。Phase 3 做设计规范时按这个流程走。
...
进入 Phase 3 时，主动扫描 `~/.Codex/skills/` 下的前端设计相关 skill：

```bash
ls ~/.Codex/skills/ | grep -iE "design|ui|theme"
```
...
如果找到 `design-consultation`，读取它的 `SKILL.md`（`~/.Codex/skills/design-consultation/SKILL.md`），把它的方法论作为 Phase 3 的"教科书"——遇到分歧时 design-consultation 的规范优先于 vibe-prd-writer 自带的 style-guide.md。

**同时扫描 marketplace / plugin 形式的设计 skill**（这些不在 `~/.Codex/skills/` 目录下，但会在当前会话的系统提示里出现）：
...
**加载** `~/.Codex/skills/vibe-prd-writer/references/module-specs.md`，按里面定义的"必含字段"和"写作规范"生成每个模块的内容。
...
> 本文件夹是面向 AI 编程 Agent（Cursor / Codex / Trae 等）的项目执行规范。
...
| 创建日期 | [日期] |
| 最后更新 | [日期] |
| 目标 Agent | Cursor / Codex / Trae / Codex |
| 技术栈 | [一句话概括] |
...
2. 让 Codex / Cursor 先读 `PRD/README.md`——它会看到导航表和加载建议
...
- **Phase 3 前必须扫描前端设计 skill 生态**——先查 `~/.Codex/skills/` 下有没有 design-consultation / web-design-pro 等更成熟的 skill，有就读它们的 SKILL.md 作为方法论参考
