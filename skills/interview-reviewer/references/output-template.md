...
面试复盘文档的 Lark-flavored Markdown 结构模板。飞书发布时使用本模板的 callout 样式，但完整目录骨架必须遵循 [`review-output-framework.md`](review-output-framework.md)。本地文件默认使用 [`local-output-template.md`](local-output-template.md)，不要输出 `<callout>` 标签。
...
| 薪资档位/岗位风险 | light-purple | purple | bar_chart |
...
<callout emoji="bar_chart" background-color="light-purple" border-color="purple">

**当前表达档位：** 约{20/30/35/40/50万+}年薪级

**目标岗位档位：** 约{根据JD/用户期望推断}
...
<callout emoji="warning" background-color="{light-green/light-orange/light-red}" border-color="{green/light-orange/red}">

**结论：** {值得去 / 谨慎去 / 不建议去}

**判断理由：**
- 成长：{是否有人带/是否有review机制}
- 权限：{是否能看数据/定优先级/推动资源}
- 目标：{试用期目标是否聚焦}
- 资源：{研发/算法/数据/运营支持是否明确}
- 薪资：{是否匹配责任和风险}
...
| Seg 10 | Part 5f 薪资档位/岗位风险判断 | docs +update append |
